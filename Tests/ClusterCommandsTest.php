...
class ClusterCommandsTest extends \PHPUnit_Framework_TestCase {
  ...
  function test_bitop_not_empty_string() {
    $infos = $this->client->info();
      foreach($infos as $info) {
  		  $version = $info['redis_version'];
  ...
  function test_bitop_not_in_place() {
  	$infos = $this->client->info();
    foreach($infos as $info) {
		  $version = $info['redis_version'];
		  if (version_compare($version, "2.6.0", "<"))
  ...
  function test_bitop_single_string() {
    $infos = $this->client->info();
    foreach($infos as $info) {
		  $version = $info['redis_version'];
		  if (version_compare($version, "2.6.0", "<"))
  ...
  function test_bitop_string_operands() {
		$infos = $this->client->info();
    foreach($infos as $info) {
		  $version = $info['redis_version'];
		  if (version_compare($version, "2.6.0", "<"))
  ...
  function test_spop() {
	...
	function test_zrangebyscore() {
		...
		$this->assertEquals($this->client->zrangebyscore('a', 2, 4, array('withscores' => TRUE)),	array('a2' => 2.0, 'a3' => 3.0, 'a4' => 4.0));
		// a non existant key should return empty list
		$this->assertEquals($this->client->zrangebyscore('b', 0, 1, array('withscores' => TRUE)), array());
	}
	...
	// HASHES
	function make_hash($key, $d){
		foreach($d as $k => $v)
			$this->client->hset($key, $k, $v);
	}

	function test_hget_and_hset() {
		// key is not a hash
		$this->client->set('a', 'a');
	...
	function test_hmset_empty_value() {
		$d = array('a' => '1', 'b' => '2', 'c' => '');
		$this->assertTrue($this->client->hmset('foo', $d));
		$this->assertEquals($this->client->hgetall('foo'), $d);
	}
	...
	function test_sort_all_options() {
		//CLUSTER
		$this->markTestSkipped();
		...
		$num = $this->client->sort('gods', array('limit' => array(2, 4),
		    'by' => 'user:*:username',
